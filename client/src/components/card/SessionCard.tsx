...
import { useDeleteSession } from "../../api/hooks/useSession";
import { Session } from "../../types/session.types";
import Button from "../UI/Button/Button";
import { useState } from "react";
import DeleteConfirmation from "../action-modals/DeleteConfirmation";

const SessionCard = ({ _id, createAt, userAgent, isCurrent }: Session) => {
    const { mutate: onDeleteSession, isPending } = useDeleteSession();
    ...
    const onClose = () => setShowDeleteModal(null);

    const onConfirmDelete = () => {
        if (showDeleteModal) {
            onDeleteSession(showDeleteModal);
        }
    };
        ...
        <>
            <div className="w-full rounded-lg py-6 px-4 bg-transparent border border-indigo-800">
                <div className="flex justify-end">
                    {isCurrent ? (
                        <div className="bg-green-500 rounded-full size-4 animate-bounce"></div>
                    ) : (
                ...
                <h3 className="text-xl font-bold mb-1">
                    {dayjs(createAt).format("dddd, MMMM D, YYYY h:mm A")}
                    &nbsp; {isCurrent && "(Current)"}
                </h3>
                <p className="text-sm text-gray-500 mb-4">{userAgent}</p>
            </div>
            <DeleteConfirmation
                open={Boolean(showDeleteModal)}
                onClose={onClose}
                deleteTitle="Are you sure you want to delete this session?"
                deleteInfo="Deleting this session will result in logging out of this
                    device."
